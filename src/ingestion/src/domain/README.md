...
`chunks.rs` menyediakan record lokal dan validator provenance/source mapping/token count. `document_wire.rs` memproyeksikan structure/chunk, `text_artifact_wire.rs` memproyeksikan mapping/page/parser output dengan offset normalized, dan `document_batch.rs` merakit serta memvalidasi ulang batch dengan typed reference closure, batas span, page issue, dan completeness eksplisit. Wire validator C01 aktif dan assembler dipakai executable worker PARSE. Reconstruction versioning pada pipeline, graph/index, dan stage worker lanjutan belum diimplementasikan.
