...
func TestWorkerCheckpointBindsRequestedStageAndOutputs(t *testing.T) {
	...
	res.Checkpoint.Stage = pb.JobStage_JOB_STAGE_CHUNK
	req.Stages = []pb.JobStage{pb.JobStage_JOB_STAGE_CHUNK}
	if err := VerifyWorkerResponse(req, res); err != nil {
		t.Fatal("CHUNK document batch should be accepted", err)
	}
	res.DocumentBatch = nil
	res.ExtractionBatch = output
	res.Checkpoint.Stage = pb.JobStage_JOB_STAGE_EXTRACT
	req.Stages = []pb.JobStage{pb.JobStage_JOB_STAGE_EXTRACT}
	if err := VerifyWorkerResponse(req, res); err != nil {
		t.Fatal("EXTRACT extraction batch should be accepted", err)
	}
	req.Stages = []pb.JobStage{pb.JobStage_JOB_STAGE_RESOLVE}
	if VerifyWorkerResponse(req, res) == nil {
		t.Fatal("extraction batch returned for RESOLVE was accepted")
	}
	res.ExtractionBatch = nil
	res.ResolutionBatch = output
	res.Checkpoint.Stage = pb.JobStage_JOB_STAGE_RESOLVE
	if err := VerifyWorkerResponse(req, res); err != nil {
		t.Fatal("RESOLVE resolution batch should be accepted", err)
	}
}
