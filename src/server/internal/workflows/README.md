...
[semantic_resolution_model.go](semantic_resolution_model.go) menyediakan `ProposeWithModel`: hydrate konteks dari EXTRACT/document/text terverifikasi, panggil gateway model, validasi proposal/revision, dan simpan request-response immutable untuk replay lintas restart. Hasilnya dapat diteruskan ke handoff registry yang sudah ada; metode ini tidak mengautentikasi reviewer atau menjalankan dispatch otomatis. Scope planner dan bukti kandidat lintas dokumen masih perlu diselesaikan. Lihat [integrasi resolusi](../../../../doc/semantic-resolution.md).
