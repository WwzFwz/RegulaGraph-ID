...
[semantic_resolution_candidates.go](semantic_resolution_candidates.go) membuat dan menyimpan kandidat dari rencana scope exact caller atau `CandidatePlanningPolicy` corpus. Jalur policy menuntut hash yang sama pada request ingest durable dan manifest kandidat, serta fingerprint konfigurasi EXTRACT yang sama dengan request. Ia memeriksa lease/checkpoint dan perubahan revision di sekitar lookup; artefak berhash serta dependency manifest terdaftar sebelum dipakai writer. Penetapan policy saat submit, identitas reviewer, dan dispatch stage produksi belum tersambung.
