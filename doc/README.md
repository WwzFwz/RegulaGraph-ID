...
storage/publication S01, serta source provenance handoff, parser PDFium, normalizer teks, parser struktur hukum, parent-aware chunk builder, boundary artefak dokumen, incremental change planner, selector timeline ketentuan, dan worker/coordinator PARSE→STRUCTURE I01 sudah aktif. Planner exact regulation identity, allocator canonical PostgreSQL, materializer `Regulation`/`ProvisionVersion`, dan binding chunk per-node K01 juga aktif sebagai library. Profiler M01 telah membandingkan tiga
engine PDF. Workflow/persistence binding dan CHUNK produksi, resolusi semantik/merge-split, stage EXTRACT–INDEX, graph/retrieval, mutasi backend Qdrant/Neo4j,
