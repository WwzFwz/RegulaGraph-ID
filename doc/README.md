...
storage/publication S01, serta source provenance handoff, parser PDFium, normalizer teks, parser struktur hukum, parent-aware chunk builder, boundary artefak dokumen, incremental change planner, selector timeline ketentuan, dan worker/coordinator PARSE→STRUCTURE I01 sudah aktif. Planner exact regulation identity dan allocator canonical PostgreSQL K01 juga aktif. Profiler M01 telah membandingkan tiga
engine PDF. Binding assignment ke `Regulation`/`ProvisionVersion` dan CHUNK produksi, resolusi semantik/merge-split, stage EXTRACT–INDEX, graph/retrieval, mutasi backend Qdrant/Neo4j,
