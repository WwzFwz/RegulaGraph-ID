...
Kerja berikutnya berjalan pada tiga jalur yang saling memasok bukti: tuntaskan D01 inventory/audit sumber, bangun G01 gold structure/temporal, dan implementasikan registry Go yang mengikat hasil STRUCTURE ke `Regulation`, `DocumentEdition`, `Provision`, serta `ProvisionVersion` sebelum CHUNK. Terminal outcome PARSE/STRUCTURE sudah durable dan crash sesudah checkpoint dapat direkonsiliasi tanpa menganggap batch parsial sukses; scheduler selanjutnya memerlukan lease heartbeat sebelum batch panjang. Setelah binding dan CHUNK, lanjutkan extraction change-event, OCR per halaman, tabel, dan exception linking. M01 tetap memilih OCR, embedding, reranker, tokenizer, serta backend native berdasarkan kualitas, latency, throughput, dan memori.
