...
Kerja berikutnya berjalan pada tiga jalur yang saling memasok bukti: tuntaskan D01 inventory/audit sumber, bangun G01 gold structure set yang dapat ditinjau manusia, dan hubungkan library parser-to-batch yang sudah tersedia ke executable worker terisolasi serta coordinator Go. Transformasi I01 berikutnya adalah reconstruction versioning/change events, routing OCR per halaman, rekonstruksi tabel, dan exception linking. M01 berlanjut untuk memilih OCR, embedding, reranker, tokenizer, serta native backend berdasarkan kualitas, latency, throughput, dan memori. Output I01 memperkaya label struktur G01; gold itu kemudian memvalidasi atau membatalkan pola heading, aturan normalisasi, ukuran/overlap chunk, dan retention pengecualian sebelum publication dan membuka K01/X01 sesuai dependency tabel. Urutan ini mengikuti dependency data dan model, bukan urutan folder. Daftar source sudah dipilih pengguna; target required tetap berlaku dan hanya status pengukurannya yang berubah setelah workload referensi siap.
