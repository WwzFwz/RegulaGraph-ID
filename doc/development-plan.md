...
## 7. Titik lanjut setelah boundary parser PDF

Kerja berikutnya berjalan pada tiga jalur yang saling memasok bukti: tuntaskan D01 inventory/audit sumber, mulai G01 gold set yang dapat ditinjau manusia, dan integrasikan parser/normalizer ke worker terisolasi serta artefak C01. Transformasi berikutnya adalah struktur hukum, routing OCR per halaman, versioning, dan chunk struktural dengan parent. M01 berlanjut untuk memilih OCR, embedding, reranker, tokenizer, serta native backend berdasarkan kualitas, latency, throughput, dan memori. Output I01 memperkaya label struktur G01; gold itu kemudian memvalidasi atau membatalkan pilihan parser/aturan normalisasi sebelum publication dan membuka K01/X01 sesuai dependency tabel. Urutan ini mengikuti dependency data dan model, bukan urutan folder. Daftar source sudah dipilih pengguna; target required tetap berlaku dan hanya status pengukurannya yang berubah setelah workload referensi siap.
