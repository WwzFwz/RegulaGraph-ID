...
Dokumen ini memecah desain lengkap RegulaGraph-ID menjadi pekerjaan komponen dan integrasi yang dapat diverifikasi. Perannya menjaga cakupan seluruh produk sambil mengurutkan pekerjaan menurut dependency nyata. Tidak ada pengurangan desain menjadi kontrak minimum atau perubahan target benchmark. Status terkini: akuisisi D01 berjalan sebagian, kontrak/validator C01 dan evaluator E01 tersedia, fondasi control-plane storage/publication S01 telah lulus pemeriksaan correctness terhadap PostgreSQL aktual, dan boundary parser PDFium I01 telah aktif sebagai library Rust. M01 secara keseluruhan, transformasi dokumen I01 setelah boundary, serta paket lain tetap belum selesai; tabel menetapkan hasil yang harus dicapai, bukan klaim kelulusan seluruh paket.

## 1. Prinsip pelaksanaan

Status D01 diperbarui: [collector dan audit PDF](acquisition.md) sudah memverifikasi integrity manifest 616 record, 619 observation, serta 650 PDF unik/2.999.240.002 byte dari batch BPK/Kemkomdigi dan percobaan JDIHN. Inventory seluruh corpus, connector JDIHN, 2.545 URL queue pending, 526 missing document reference, klasifikasi gold format, dan gold dataset belum selesai. C01, E01, serta control-plane S01 sudah direalisasikan sesuai laporan milestone; candidate identity inventory tetap unverified dan bukan canonical registry. Profiler M01 mengelompokkan 120 sampel menjadi 93 text, 19 scan, dan 8 mixed candidate secara heuristic serta memilih PDFium. Boundary I01 kini memverifikasi hash/identity dan menghasilkan teks, locator, status halaman, serta manifest; OCR, struktur gold, mapping raw-normalized, worker isolation, RSS, model semantic, dan benchmark produksi tetap belum diukur.
...
Kerja berikutnya berjalan pada tiga jalur yang saling memasok bukti: tuntaskan D01 inventory/audit sumber, mulai G01 gold set yang dapat ditinjau manusia, dan integrasikan boundary PDFium ke worker terisolasi serta transformasi dokumen I01. Transformasi berikutnya adalah mapping raw-normalized, struktur hukum, routing OCR per halaman, versioning, dan chunk struktural dengan parent. M01 berlanjut untuk memilih OCR, embedding, reranker, tokenizer, serta native backend berdasarkan kualitas, latency, throughput, dan memori. Output PDFium I01 memperkaya label struktur G01; gold itu kemudian memvalidasi atau membatalkan pilihan parser sebelum publication dan membuka K01/X01 sesuai dependency tabel. Urutan ini mengikuti dependency data dan model, bukan urutan folder. Daftar source sudah dipilih pengguna; target required tetap berlaku dan hanya status pengukurannya yang berubah setelah workload referensi siap.
