...
Dokumen ini memecah desain lengkap RegulaGraph-ID menjadi pekerjaan komponen dan integrasi yang dapat diverifikasi. Perannya menjaga cakupan seluruh produk sambil mengurutkan pekerjaan menurut dependency nyata. Status terkini: D01 berjalan sebagian; C01/E01 dan fondasi storage/publication S01 tersedia; I01 memiliki library PDFium sampai persistence `DocumentBatch`, incremental planner, selector timeline, executable worker gRPC tahap PARSE, coordinator Go yang menyimpan artifact/checkpoint fenced, serta retry availability dan max-attempt durable. M01 secara keseluruhan, stage STRUCTURE–INDEX, extraction change-event, canonical registry, dan full-rebuild equivalence tetap belum selesai; tabel menetapkan hasil yang harus dicapai, bukan klaim kelulusan seluruh paket.

## 1. Prinsip pelaksanaan

Status D01 diperbarui: [collector dan audit PDF](acquisition.md) sudah memverifikasi integrity manifest 616 record, 619 observation, serta 650 PDF unik/2.999.240.002 byte dari batch BPK/Kemkomdigi dan percobaan JDIHN. Inventory seluruh corpus, connector JDIHN, 2.545 URL queue pending, 526 missing document reference, klasifikasi gold format, dan gold dataset belum selesai. C01, E01, serta control-plane S01 sudah direalisasikan. I01 memverifikasi hash/identity, menghasilkan teks/locator/status halaman, mapping byte canonical, struktur/chunk library, reference closure, persistence content-addressed, incremental plan, serta timeline as-of. Worker gRPC menjalankan PARSE dengan deadline/fence/cancellation dan mengembalikan checkpoint yang terikat artifact; coordinator Go mendaftarkan output serta state handoff dan menjadwalkan retry transient secara durable. OCR, tabel, extraction change-event, canonical registry, temporal/structure gold, stage worker lanjutan, full-rebuild equivalence, RSS, tokenizer/model produksi, dan benchmark produksi tetap belum diukur.
...
Kerja berikutnya berjalan pada tiga jalur yang saling memasok bukti: tuntaskan D01 inventory/audit sumber, bangun G01 gold structure/temporal, dan lanjutkan handoff PARSE menuju STRUCTURE dengan identitas edisi/regulasi/ketentuan eksplisit agar STRUCTURE dan CHUNK tidak membuat ID sintetis. Scheduler perlu menambah lease heartbeat sebelum batch panjang serta claim owner untuk stage berikutnya. Setelah itu lanjutkan extraction change-event, canonical registry, OCR per halaman, tabel, dan exception linking. M01 tetap memilih OCR, embedding, reranker, tokenizer, serta backend native berdasarkan kualitas, latency, throughput, dan memori.
