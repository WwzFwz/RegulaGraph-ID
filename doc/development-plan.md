...
Dokumen ini memecah desain lengkap RegulaGraph-ID menjadi pekerjaan komponen dan integrasi yang dapat diverifikasi. Perannya menjaga cakupan seluruh produk sambil mengurutkan pekerjaan menurut dependency nyata. Tidak ada pengurangan desain menjadi kontrak minimum atau perubahan target benchmark. Status terkini: akuisisi D01 berjalan sebagian, kontrak/validator C01 dan evaluator E01 tersedia, fondasi control-plane storage/publication S01 telah lulus pemeriksaan correctness terhadap PostgreSQL aktual, dan jalur library I01 dari PDFium sampai proyeksi/persistence `TextArtifact`, assembly/persistence `DocumentBatch`, planning perubahan incremental, serta validasi/seleksi timeline ketentuan telah aktif. M01 secara keseluruhan, executable worker, extraction change-event, canonical registry, full-rebuild equivalence, dan bagian I01 lain serta paket berikutnya tetap belum selesai; tabel menetapkan hasil yang harus dicapai, bukan klaim kelulusan seluruh paket.

## 1. Prinsip pelaksanaan

Status D01 diperbarui: [collector dan audit PDF](acquisition.md) sudah memverifikasi integrity manifest 616 record, 619 observation, serta 650 PDF unik/2.999.240.002 byte dari batch BPK/Kemkomdigi dan percobaan JDIHN. Inventory seluruh corpus, connector JDIHN, 2.545 URL queue pending, 526 missing document reference, klasifikasi gold format, dan gold dataset belum selesai. C01, E01, serta control-plane S01 sudah direalisasikan sesuai laporan milestone; candidate identity inventory tetap unverified dan bukan canonical registry. Profiler M01 mengelompokkan 120 sampel menjadi 93 text, 19 scan, dan 8 mixed candidate secara heuristic serta memilih PDFium. I01 kini memverifikasi hash/identity, menghasilkan teks/locator/status halaman, membangun mapping byte raw-normalized canonical, mendeteksi hierarchy heading konservatif, menghasilkan chunk tervalidasi, memproyeksikan record ke C01, memeriksa reference closure/completeness batch, menyimpan payload lokal content-addressed, membentuk `UpdatePlan` deterministik, serta memvalidasi dan memilih versi ketentuan as-of tanpa mengganti unknown date dengan observation time. OCR, tabel, extraction change-event, canonical registry, temporal gold, structure gold, executable worker, full-rebuild equivalence, RSS, tokenizer/model produksi, dan benchmark produksi tetap belum diukur.
...
Kerja berikutnya berjalan pada tiga jalur yang saling memasok bukti: tuntaskan D01 inventory/audit sumber, bangun G01 gold structure dan temporal set yang dapat ditinjau manusia, dan hubungkan library parser-to-batch yang sudah tersedia ke executable worker terisolasi serta coordinator Go. Transformasi I01 berikutnya adalah extraction change-event ke timeline versioning yang sudah aktif, canonical registry, routing OCR per halaman, rekonstruksi tabel, dan exception linking. M01 berlanjut untuk memilih OCR, embedding, reranker, tokenizer, serta native backend berdasarkan kualitas, latency, throughput, dan memori. Output I01 memperkaya label struktur/temporal G01; gold itu kemudian memvalidasi atau membatalkan pola heading, aturan normalisasi, interval hukum, ukuran/overlap chunk, dan retention pengecualian sebelum publication dan membuka K01/X01 sesuai dependency tabel. Urutan ini mengikuti dependency data dan model, bukan urutan folder. Daftar source sudah dipilih pengguna; target required tetap berlaku dan hanya status pengukurannya yang berubah setelah workload referensi siap.
