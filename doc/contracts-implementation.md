...
Tujuh proto produksi pada [v1](../src/contracts/proto/regulagraph/v1/README.md) dan [evaluation.proto](../evaluation/datasets/evaluation.proto) mendefinisikan 157 message, 31 enum, dan empat service descriptor. `common` memiliki primitive/presence/ID/provenance/dependency; `documents` memiliki sumber, struktur, versi dan chunk; `graph` memiliki identity/assertion/support/path/registry; `evidence` memiliki indeks, retrieval dan context; `answers` memiliki claim/citation/stream; `jobs` memiliki worker/ledger/publication; `inference` memiliki model batch dan semantic gateway. Evaluasi merujuk tipe produksi yang sama.
...
Ikuti [implementation-guide](implementation-guide.md) dan [verification](verification.md). E01 membangun evaluator required gate; S01 menghubungkan guard ke storage dan transaksi nyata. N01/A01 menerapkan transport/model dan citation resolver. C01 belum membuktikan quality model, end-to-end performance atau seluruh invariant pada data produksi. Target benchmark tetap REQUIRED_UNMEASURED sampai run yang memenuhi prasyarat dilaksanakan.
