...
Layanan C++ belum ada; scaffold saat ini hanya static library. Rust memiliki executable worker Tonic loopback untuk batch PARSE, STRUCTURE, CHUNK, dan EXTRACT, dengan Go client serta coordinator durable yang memvalidasi deadline, attempt/fence, bytes output, dependency, evidence, dan checkpoint. STRUCTURE tidak menciptakan ID ketentuan; BIND Go memasok provision/version registry-owned sebelum CHUNK membentuk unit parent-aware; EXTRACT menghasilkan proposal graph provisional yang belum merupakan canonical assignment. Tidak ada RPC terpisah untuk fusion, filtering, context builder, atau pemeriksaan citation.
