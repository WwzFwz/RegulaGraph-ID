...
Build Go memvalidasi package, collector, storage/control-plane, client Worker, serta daemon coordinator PARSE→STRUCTURE→BIND→CHUNK; entry point API query tetap scaffold. Rust menyediakan executable worker Tonic untuk PARSE dengan PDFium, STRUCTURE dari text/mapping terverifikasi, dan CHUNK registry-bound dengan tokenizer Hugging Face hash-pinned. Go BIND menyelesaikan exact canonical identity dan materialisasi regulation/provision-version; coordinator memverifikasi bytes/dependency/checkpoint setiap output. C++ masih static library tanpa ONNX runtime/model. C01 menyediakan 157 message, 31 enum, empat service descriptor, codegen dan validator. Stage EXTRACT–INDEX, inference, dan deployment produksi belum aktif. Packaging Python hanya mencakup evaluation dan tooling.
