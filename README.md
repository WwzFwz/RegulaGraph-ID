...
Build Go memvalidasi package, collector, storage/control-plane, client Worker, coordinator PARSE→STRUCTURE→BIND→CHUNK, serta Semantic.ExtractBatch Gateway; entry point API query tetap scaffold. Rust menyediakan worker Tonic untuk PARSE, STRUCTURE, CHUNK, dan EXTRACT terkonfigurasi. Go BIND menyelesaikan exact identity dan coordinator memverifikasi bytes/dependency/checkpoint sampai CHUNK. C++ masih static library tanpa ONNX runtime/model. C01 menyediakan 159 message, 31 enum, empat service descriptor, codegen dan validator. Commit durable EXTRACT, RESOLVE–INDEX, provider/model produksi, dan deployment produksi belum aktif. Packaging Python hanya mencakup evaluation dan tooling.
